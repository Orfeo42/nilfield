...
3. **Done.** The plugin package lives at `plugin/` (`github.com/Orfeo42/nilfield/plugin`), registering `nilfield` with `github.com/golangci/plugin-module-register` (resolved to v0.1.2) in `init()`. It is a separate package from the module root, so a `.custom-gcl.yml` plugin entry for this module needs an explicit `import: 'github.com/Orfeo42/nilfield/plugin'` line alongside `module`/`path` — omitting it makes `golangci-lint custom` look for a plugin registration at the module root, which does not exist. The standalone binary in `cmd/nilfield` does not import `plugin/`, so it carries no `plugin-module-register` dependency.
...
4. **Pin `golang.org/x/tools`.** `golangci-lint custom` compiles the plugin into the golangci-lint binary, so both link one copy of `x/tools`. A plugin pinned far from golangci-lint's own version will either fail to build or silently take golangci-lint's version through MVS. Test the plugin against the golangci-lint version named in `.custom-gcl.yml`, not just against whatever `go.mod` resolves.
   ...
   Measured: this module pins `golang.org/x/tools v0.37.0`; golangci-lint v2.13.2's own `go.mod` requires `golang.org/x/tools v0.49.0`. The two do not agree, and the pin was deliberately left alone per this section's own instruction, since the end-to-end build below succeeded anyway — `go build -buildmode=plugin`-style hard version equality is not the mechanism here; `golangci-lint custom` builds a single Go module graph and Go's MVS resolves the shared `x/tools` requirement up to the higher version (`v0.49.0`) for the `custom-gcl` binary, and the plugin code compiled and ran correctly against it. If a future `x/tools` release breaks something nilfield's code depends on, that will surface as a `golangci-lint custom` build failure, not a silent divergence.
...
That claim is now measured, not just read from source. A `custom-gcl` was built from the `.custom-gcl.yml` in step 3 (`version: v2.13.2`, plugin module path pointed at this checkout, `import: 'github.com/Orfeo42/nilfield/plugin'`) via `golangci-lint custom`. Against it, a scratch two-package module was run — a `dep` package exporting `Request` with a `Validate() error` method that rejects a nil `P` field, and a `main` package calling `Validate`, checking the error, then dereferencing both `r.P.N` (validator-proven) and `r.Q.N` (not checked by `Validate`) — with `nilfield` enabled as a `type: module` custom linter in `.golangci.yml`. `./custom-gcl run ./...` reported exactly one finding, `r.Q may be nil here` on the unchecked field, and nothing on the validator-proven `r.P.N`: the same split `analysistest` enforces on `testdata/src/validator`. Re-running with `settings.custom.nilfield.settings.exclude-paths` set to a fragment matching `main.go` dropped that finding too (`0 issues.`), confirming plugin settings reach the analyzer's `-exclude-paths` flag through `register.DecodeSettings`. Fact propagation across the package boundary, and settings propagation through the plugin, both hold under golangci-lint's real runner, not just in `analysistest`.
...
`nilfield` reports a nil pointer or a nil interface being dereferenced, and nothing else. Which other linters a consumer runs alongside it, and what those cover, is the consumer's configuration to decide; this project makes no claim about it.
