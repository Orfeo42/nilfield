...
//nilfixture:shared
func (i *inner) safeOnNil() bool { return i == nil }
...
//
// FALSE POSITIVE: -assert-package names an imported package, so an assert helper
// declared in the package under analysis is invisible to it.
//
...
//
// FALSE POSITIVE: only if/else and early returns are read as guards; a switch is
// not.
//
...
func nilInterfaceCall(o *outer) int { return o.iface.Do() }
...
func uncheckedTypeAssert(v any) int { return v.(*inner).n }
...
//nilfixture:shared
func mayReturnNil(ok bool) *inner {
	if ok {
		return &inner{}
	}
...
func missingMapPointerValue(m map[string]*inner) int { return m["k"].n }

//nilhazard:nil-element-in-slice sites=1
func nilElementInSlice(s []*inner) int { return s[0].n }
...
func promotedFieldOnNilEmbedded(o *outer) int { return o.e }
...
func returnNilNil() (*inner, error) { return nil, nil }
...
func nilErrorMessage(err error) string { return err.Error() }
...
func doublePointer(pp **inner) int { return (*pp).n }

// pp, *pp, and the next field.
//
//nilhazard:double-pointer-chain sites=3
func doublePointerChain(pp **inner) int { return (*pp).next.n }
